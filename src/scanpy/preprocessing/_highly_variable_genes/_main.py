...
def highly_variable_genes(  # noqa: PLR0913
    ...
    *,
    ...
    For :class:`~dask.array.Array` inputs, chunking along the observation (cell) axis
    is supported. Column-chunked :class:`~scipy.sparse.csc_matrix` /
    :class:`~scipy.sparse.csc_array` blocks (feature axis chunked, observation axis
    whole) are also supported for all flavors. Feature-axis-chunked dense or
    :class:`~scipy.sparse.csr_matrix` / :class:`~scipy.sparse.csr_array` inputs, and
    ``csc`` arrays chunked on both axes, raise :class:`ValueError`.
