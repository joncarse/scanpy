...
def _as_dask_csc_uneven_col_chunks(x: np.ndarray):
    import dask.array as da

    # last gene chunk smaller than the others (common with gene-block stores)
    return da.from_array(sps.csc_matrix(x), chunks=(-1, (3, 3, 2)))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            _as_dask_csc_uneven_col_chunks,
            id="dask_csc_uneven_col_chunks",
            marks=needs.dask,
        ),
    ],
)
...
def test_clip_square_sum_rejects_feature_and_obs_chunked(
    ...
    """Feature-chunked path requires the observation axis to be a single chunk."""
    import dask.array as da

    x, clip_val = data
    dual = da.from_array(sps.csc_matrix(x), chunks=(5, 3))
    assert dual.numblocks[0] > 1
    assert dual.numblocks[1] > 1
